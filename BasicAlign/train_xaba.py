...
def main():
    ...
    #print(opt['stage'], "fffffffffffffffffffffff")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                ...
                if rank<=0:
                    results = model.test(opt)
                    val_message = ''
                    for logs in results:
                        for k, v in logs.items():
                            if best_psnr_loss < v:
                                best_psnr_step = current_step
                                best_psnr_loss = v
                            val_message += '{:s}: {:.4e} '.format(k, v) 
                        val_message += '\n'
                    
                    # log
                    logger.info('# Validation # avg psnr result: {:s}'.format(val_message))
                    logger.info('# Validation best psnr is : {:.4e}, best step is: {:8,d}'.format(best_psnr_loss, best_psnr_step))
                    # tensorboard logger
                    #if opt['use_tb_logger'] and 'debug' not in opt['name']:
