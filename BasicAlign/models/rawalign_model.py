...
class RawAlignModel(BaseModel):
    def __init__(self, opt):
        super(RawAlignModel, self).__init__(opt)
    ...
    def optimize_parameters(self, step):
        ...
        if self.loss_type == 'fs':
            ...
            l_pix = self.l_pix_w * self.cri_pix(self.img_align, self.img_base)

        if self.ssim_flag:
            l_ssim = self.ssim_loss(self.img_align, self.img_base)
        ...
        if self.perp_flag:
            ...
            bases = torch.cat([self.img_base[:, 0:1, :, :], (self.img_base[:, 1:2, :, : ] + self.img_base[:, 2:3, :, : ])/2, self.img_base[:, 3:4, :, : ]], 1)
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        #out_dict['img_ref'] = self.img_ref.detach()[0].float().cpu()
