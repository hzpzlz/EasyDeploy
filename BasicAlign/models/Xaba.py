...
from models.loss import CharbonnierLoss, FSLoss, GradientLoss, Sequence_Loss, Sequence_Warp, PWC_Loss
...
class XabaModel(BaseModel):
    def __init__(self, opt):
        super(XabaModel, self).__init__(opt)
        ...
        self.opt = opt
        self.raw_input = opt['network_G']['raw_input']
    ...
    def fetch_optimizer(self, opt, model):
        ...
        optimizer = optim.Adam(model.parameters(), lr=opt['train']['lr_G'],
                    weight_decay=opt['train']['weight_decay'], betas=(opt['train']['beta1'], opt['train']['beta2']))
        if opt['train']['lr_scheme'] == 'MultiStepLR': 
            scheduler = lr_scheduler.MultiStepLR_Restart(optimizer, opt['train']['lr_steps'],
            ...
            scheduler = lr_scheduler.CosineAnnealingLR_Restart(
                            optimizer, opt['train']['T_period'], eta_min=opt['train']['eta_min'],
                            restarts=opt['train']['restarts'], weights=opt['train']['restart_weights'])
    ...
    def optimize_parameters(self):
        ...
        #if self.raw_input==False:
        #    img_mean1 = torch.mean(self.img1, dim=1, keepdim=True)
        #    img_mean2 = torch.mean(self.img2, dim=1, keepdim=True)
        #    self.img1 = torch.cat([self.img1, img_mean1], dim=1)
        #    self.img2 = torch.cat([self.img2, img_mean2], dim=1)
        self.align_img = self.netG(self.img2, self.img1)

        loss_total = 0
        if self.flow_loss_type in ['sequence_loss', 'pwc_loss']:
        ...
        if self.pixel_loss_type is not None:
            ...
            pix_loss = self.l_pix_w * self.cri_pix(self.img1, self.align_img)
            loss_total += pix_loss
        ...
        with torch.no_grad():
            ...
            for val_dataset in opt_val['name']:
                if val_dataset == 'chairs':
                    ...
                    val_res = evaluate.validate_chairs_img(self.netG.module, val_dataroot, raw_input=self.raw_input)
                    ...
                    val_res = evaluate.validate_kitti_img(self.netG.module, val_dataroot, raw_input=self.raw_input)
                    result.append(val_res)
