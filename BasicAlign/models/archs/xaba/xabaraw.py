...
from .module import FeatureExtractorRes, PAFBRAW, PAFBRAW_n5

class Net(nn.Module):
    def __init__(self, fe=32, fm=16, factor=20):
        super(Net, self).__init__()
        self.relu = nn.LeakyReLU(0.1, inplace=True)
        self.fe1 = nn.Conv2d(4, 4, 5, 1, 2)
        self.fe2 = FeatureExtractorRes(fe=fe, fm=fm, factor=factor, ch_in=4)
        self.fe4 = FeatureExtractorRes(fe=fe, fm=fm, factor=factor, ch_in=4)
        self.conv1 = nn.Conv2d(4, 4*4, 3, 1, 1)
        self.conv2 = nn.Conv2d(4, 4*16, 3, 1, 1)
...
class NetLV3(nn.Module):
    def __init__(self, fe=32, fm=16, factor=20):
        super(NetLV3, self).__init__()
        self.relu = nn.LeakyReLU(0.1, inplace=True)
        self.fe1 = nn.Conv2d(4, 4, 5, 1, 2)
        self.fe2 = FeatureExtractorRes(fe=fe, fm=fm, factor=factor, ch_in=4)
        self.fe4 = FeatureExtractorRes(fe=fe, fm=fm, factor=factor, ch_in=4)
        self.fe8 = FeatureExtractorRes(fe=fe, fm=fm, factor=factor, ch_in=4)
        self.conv1 = nn.Conv2d(4, 4*4, 3, 1, 1)
        self.conv2 = nn.Conv2d(4, 4*16, 3, 1, 1)
        self.conv3 = nn.Conv2d(4, 4*64, 3, 1, 1)
        self.shuffle2 = nn.PixelShuffle(2)
        self.shuffle4 = nn.PixelShuffle(4)
        self.shuffle8 = nn.PixelShuffle(8)

        self.last = PAFBRAW_n5(fe=16, fm=32)
    ...
    def forward(self, x1_raw, x2_raw): #x1_raw:ref, x2_raw:target
        ...
        x1_d8 = F.interpolate(x1_d4, scale_factor=0.5)
        x2_d8 = F.interpolate(x2_d4, scale_factor=0.5)

        l1_align = self.fe1(x1_raw)
        l2_align = self.fe2(x1_d2, x2_d2)
        l4_align = self.fe4(x1_d4, x2_d4)
        l8_align = self.fe8(x1_d8, x2_d8)

        l2_u2 = self.shuffle2(self.conv1(l2_align))
        l4_u4 = self.shuffle4(self.conv2(l4_align))
        l8_u8 = self.shuffle8(self.conv3(l8_align))

        Ic = torch.cat([l1_align, l2_u2, l4_u4, l8_u8, x2_raw], dim=1)
