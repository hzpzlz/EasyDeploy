...
class FeatureExtractor(nn.Module):
    def __init__(self, fe=16, fm=32, factor=16, ch_in=3):
...
class PAFB(nn.Module):
    def __init__(self, fe=16, fm=32, ch_in=12):
        super(PAFB, self).__init__()
        self.conv1 = nn.Conv2d(ch_in, fe, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(fe, fm, 3, 1, 1, bias=True)
        self.conv3 = nn.Conv2d(fm, 3, 3, 1, 1, bias=True)
        self.relu = nn.LeakyReLU(0.1,inplace=True)
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, x):
        ...
        out = mask[:, 0:1, :, :] * x[:, 0:3, :, :] + mask[:, 1:2, :, :] * x[:, 3:6, :, :] + mask[:, 2:3, :, :] * x[:, 6:9, :, :]
        return out

class PAFBRAW(nn.Module):
    def __init__(self, fe=16, fm=32, ch_in=16):
        super(PAFBRAW, self).__init__()
        self.conv1 = nn.Conv2d(ch_in, fe, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(fe, fm, 3, 1, 1, bias=True)
        self.conv3 = nn.Conv2d(fm, 3, 3, 1, 1, bias=True)
        self.relu = nn.LeakyReLU(0.1,inplace=True)
    ...
    def forward(self, x):
        ...
        mask = self.softmax(x3)
        out = mask[:, 0:1, :, :] * x[:, 0:4, :, :] + mask[:, 1:2, :, :] * x[:, 4:8, :, :] + mask[:, 2:3, :, :] * x[:, 8:12, :, :]
        return out
...
if __name__ == '__main__':
    a = torch.rand(16, 3, 320, 320)
    b = torch.rand(16, 3, 320, 320)
    xx = FeatureExtractor()
    out = xx(a, b)
    print(torch.mean(a), torch.mean(b), torch.mean(out))
