...
class HRAFT(nn.Module):
    ...
    def forward(self, image1, image2, flow_init=None, upsample=True, test_mode=False):
        ...
        flow_global_init = self.initialize_flow(image1 ,True)[0]

        flow_global = grid_global - flow_global_init
        down_flow = self.downflow8(flow_global)
        ...
        coords1 = down_flow + coords1
        delta_flow_gloal = torch.zeros_like(down_flow).to(coords1.device)

        flow_predictions = []
        for itr in range(self.iters):
            coords1 = coords1.detach()
            corr = corr_fn(coords1) # index correlation volume  和corr一起估计 论文中连到L的那部分
            ...
            # F(t+1) = F(t) + \Delta(t)
            coords1 = coords1 + delta_flow + delta_flow_gloal   #论文中 0->那一列 更新
            ...
            img2_warp = bilinear_sampler(image2, flow_up.permute(0, 2, 3, 1))
            grid_tmp, pred_tmp_I2 = get_grid(img2_warp, H_mat, out_size)
            #pred_I2_fea = self.fnet(pred_tmp_I2)
            grid_flow_tmp = self.downflow8(grid_tmp - flow_global_init)
            delta_flow_gloal = grid_flow_tmp - down_flow
            down_flow = grid_flow_tmp
