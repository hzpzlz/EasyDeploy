...
class STN_Unet_v3(nn.Module):
    def __init__(self, cpf=3, n_frame=5, out_nc=3):
        super(STN_Unet_v3, self).__init__()
        self.unet1 = Unet(if_stn=True, in_nc=cpf*2)
        self.unet2 = Unet(if_stn=True, in_nc=cpf*2)
        self.unet3 = Unet(if_stn=False)
        self.unet4 = Unet(if_stn=True, in_nc=cpf*2)
        self.unet5 = Unet(if_stn=True, in_nc=cpf*2)

    def forward(self, x):
        f1, f2, f3, f4, f5 = torch.split(x, 3, 1)
        f1 = self.unet1(torch.cat([f1, f3], 1))
        f2 = self.unet2(torch.cat([f2, f3], 1))
        f3_ = self.unet3(f3)
        f4 = self.unet4(torch.cat([f4, f3], 1))
        f5 = self.unet5(torch.cat([f5, f3], 1))
