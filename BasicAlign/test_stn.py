...
for test_loader in test_loaders:
    ...
    for data in test_loader:
        ...
        #output_model = visuals['output_stn']  # uint8
        ...
        for i in range(opt['n_frames']):
            ...
            img_in = util.tensor2img((data['data'])[:, (i*cpf):((i+1)*cpf), :, :])
            img_out = util.tensor2img(output_model[i])
            if i!=2:
                psnr_tmp = util.calculate_psnr(util.tensor2img(sample[:, (mid*cpf):((mid+1)*cpf), :, :]), img_out)
                print(psnr_tmp, "****************")
                ssim_tmp = util.calculate_ssim(util.tensor2img(sample[:, (mid*cpf):((mid+1)*cpf), :, :]), img_out)
                psnr+=psnr_tmp
                ssim+=ssim_tmp

            if opt['save_img']:
                save_img_out_path = osp.join(save_path, img_name + '_frame_{:d}_{:s}_out_{:s}.jpg'.format(i, osp.basename(opt['path']['pretrain_model_G']).split('/')[-1].split('.')[0], opt['name']))
                save_img_in_path = osp.join(save_path, img_name + '_frame_{:d}_{:s}_in_{:s}.jpg'.format(i, osp.basename(opt['path']['pretrain_model_G']).split('/')[-1].split('.')[0], opt['name']))
