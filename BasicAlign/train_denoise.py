    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                if opt['model'] in ['sr', 'udvd', 'denoise'] and rank <= 0:  # image restoration validation
                    ...
                    for val_data in val_loader:
                        ...
                        if opt['save_img']:
                             util.save_img(util.tensor2img(output_model), save_img_path)

                        # calculate PSNR
                        #sr_img, gt_img = util.crop_border([sr_img, gt_img], opt['scale'])
                        mid = opt['datasets']['val']['n_frames'] // 2
                        cpf = opt['network_G']['channels_per_frame']
                        avg_psnr += metrics.psnr(sample[:, (mid*cpf):((mid+1)*cpf), :, :], output_model)
