...
for root_dir in os.listdir(root_path): 
    ...
    for file in os.listdir(file_path):
        if file.endswith('_roll.jpg'):
        #if file.endswith('2.alignraw_ai.jpg'):
            #print(file)
            basename = file.split('.')[0]
            outfilename = basename + flag + '.jpg'
            #outfilename = root_dir + flag + '.jpg'
            old_path = os.path.join(file_path, file)
            new_path = os.path.join(out_path, outfilename)
